...
public class PlayerListener implements Listener {
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if (!quickCommandsProtectionManager.isAllowed(player.getName())) {
            event.setCancelled(true);
            bukkitService.runTask(player, task -> player.kickPlayer(messages.retrieveSingle(player, MessageKey.QUICK_COMMAND_PROTECTION_KICK)));
            return;
        }
    ...
    public void onPlayerTeleport(PlayerTeleportEvent event) {
        if (!settings.getProperty(RestrictionSettings.CANCEL_ALL_TELEPORT)) {
            return;
        }
        if (listenerService.shouldCancelEvent(event)) {
            event.setCancelled(true);
    ...
    public void onPlayerInventoryOpen(InventoryOpenEvent event) {
        ...
        if (listenerService.shouldCancelEvent(player)
             ...
             */
            bukkitService.runTaskLater(player, task -> player.closeInventory(), 1);
        }
    }
